...
?>
    <div class="row">
        <div class="col-sm-9">
            ...
            //rendering the table
            echo "<table class='transactions table table-striped table-hover' id='transactions'>";
            echo "<tr>
                        <th field='data' width='175'>Data tranzactiei</th>
                        <!--<th>Data inregistrarii</th>-->
                        <th id='categoria' field='categoria'>Categoria</th>
                        <th field='destinatia'>Destinatia</th>
                        <th field='beneficiar' width='100'>Beneficiar</th>
                        <th field='suma' id='suma' width='80'>Suma</th>
                </tr>";
            $i = 0;
            $total_zi = 0;
            $total_sapt = 0;
        ...
        <td class='sum'>" . number_format($cel["suma"], 2, '.', ' ')  . "</td>
                        ...
                        if (date('d', $table[$i]['data_op']) !== date('d', $table[$i - 1]['data_op']) || !$table[$i]['data_op']) {
                            ...
                            echo "<tr style='' class='alert alert-success'><td colspan='3'><b><i>Total pe ziua " . date('d-m-Y', $cel['data_op']) . "</i></b></td><td colspan='2' class='sum'><b><i>" . number_format($total_zi, 2, '.', ' ') . "</i></b></td></tr>";
                ...
                if (date('w', $z) == '0' && !$total_sapt == 0 || $z == $last_day) {
                    echo "<tr style='' class='alert alert-info'><td colspan='3'><b><i>Total saptamanal</i></b></td><td colspan='2' class='sum'><b><i>" . number_format($total_sapt, 2, '.', ' ') . "</i></b></td></tr>";
                    $total_sapt = 0;
                }
            }
            echo "<tr style='' class='alert alert-danger'><td colspan='3'><b><i>Total perioada</i></b></td><td colspan='2' class='sum'><b><i>" . number_format($total_perioada, 2, '.', ' ') . "</i></b></td></tr>";
            echo "</table>";
            ?>
        </div>
